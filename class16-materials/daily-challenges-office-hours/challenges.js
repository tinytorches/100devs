// Please create a function that takes in two numbers. Print to the console the division of these two numbers rounded down.
...
function addFirstAndLastValArray(arr) {
    console.log( arr[0] + arr[arr.length - 1] ) // .length - 1 will always give us the last value in an array
}
...
function addNameToList(nameForList) {
    namesList.push(nameForList)  // add this person to the list
...
// jump ahead on the reading? map() could help
function newArrOnlyOdd(arr) {
    let newArr = []
    for ( let i = 0; i < arr.length; i++) {
        if ( arr[i] % 2 !== 0 ) { // arr[i] refers to your argument 
            newArr.push(arr[i])
        } 
    }
    return newArr
...
newArrOnlyOdd([1, 2, 3, 4, 5])
 ...
 // Please create a function that takes in a number. Print all the numbers from 1 to that number in the console, but skip any number that when multiplied by three is divisible by 2 evenly (quick edit) .
function numMultiAndDivi(num) {
    for ( let i = 1; i <= num; i++) { // we're going to go up by 1 each time it runs
        if ( i * 3 % 2 !== 0 ) { // is it not disivible by 2? Print it to the console
            console.log(i)
        }
    }
}
numMultiAndDivi(100) // 100 goes into 'num' as an argument